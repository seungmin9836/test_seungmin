...
public class ServiceCenter1vs1_dao {
		...
		public ServiceCenter1vs1_dao() {
			try {
				...
				dataSource = (DataSource) context.lookup("java:comp/env/jdbc/servicecenter"); //java:comp/env는 BListCommand다!
			}catch (Exception e) {
				e.printStackTrace();
			}
		...
		// 전체 검색 (dao는 DB에서 가져오는 거고 jsp에 보내줘야되기때문에 dto를 만들러 가야됨)
			public ArrayList<ServiceCenter1vs1_dto> Inquire() {
				...
				try {
					...
					String query = "select iCode, iTitle, iContent, iDate, iReply, iViews from inquire"; 
					...
					while(resultSet.next()) {
						...
						int iViews = resultSet.getInt("iViews");
						
						ServiceCenter1vs1_dto dto = new ServiceCenter1vs1_dto(iCode, iTitle, iContent, iDate, iReply, iViews); // dto에 다 넣은것
